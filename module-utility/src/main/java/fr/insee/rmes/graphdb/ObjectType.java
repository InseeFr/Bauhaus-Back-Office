...
public enum ObjectType {
    ...
    DIMENSION_PROPERTY(
            "dimensionProperty", QB.DIMENSION_PROPERTY, STRUCTURES_COMPONENTS_BASE_URI, s -> s + "dimension"),
    ...
    ObjectType(
            ...
            final IRI uri,
            final String baseUriPropertyName,
            final UnaryOperator<String> baseUriModifier) {
        this.uri = uri;
    ...
    public static Optional<ObjectType> getEnumByLabel(final String labelType) {
        return Arrays.stream(ObjectType.values())
                .filter(e -> e.labelType.equals(labelType))
                .findAny();
    }
    ...
    public static ObjectType getEnum(final IRI uri) {
        return Arrays.stream(ObjectType.values())
                .filter(e -> Objects.equals(e.uri, uri))
                .findAny()
