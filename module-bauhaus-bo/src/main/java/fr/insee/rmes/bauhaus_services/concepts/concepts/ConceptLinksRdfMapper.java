...
public class ConceptLinksRdfMapper {
    ...
    private void addTripleReplacedByMatch(IRI conceptURI, List<String> conceptsIDToLink, Model model) {
        conceptsIDToLink.forEach(conceptIDToLink -> {
            model.add(
                    conceptURI, DCTERMS.IS_REPLACED_BY, RdfUtils.conceptIRI(conceptIDToLink), RdfUtils.conceptGraph());
    ...
    private void addTripleSucceed(IRI conceptURI, List<String> conceptsIDToLink, Model model) {
        conceptsIDToLink.forEach(conceptIDToLink -> {
            model.add(conceptURI, DCTERMS.REPLACES, RdfUtils.conceptIRI(conceptIDToLink), RdfUtils.conceptGraph());
            model.add(
                    RdfUtils.conceptIRI(conceptIDToLink), DCTERMS.IS_REPLACED_BY, conceptURI, RdfUtils.conceptGraph());
        });
    }

    private void addTripleCloseMatch(IRI conceptURI, List<String> urn, Model model) {
        urn.forEach(
                urnToAdd -> model.add(conceptURI, SKOS.CLOSE_MATCH, RdfUtils.toURI(urnToAdd), RdfUtils.conceptGraph()));
    }
    ...
    private void addTripleReferences(IRI conceptURI, List<String> conceptsIDToLink, Model model) {
        conceptsIDToLink.forEach(conceptIDToLink -> model.add(
                conceptURI, DCTERMS.REFERENCES, RdfUtils.conceptIRI(conceptIDToLink), RdfUtils.conceptGraph()));
    }
