...
public class StructureRepository extends RdfService {
        ...
        for (int i = 0; i < structures.length(); i++) {
            ...
            current.put(
                    ...
                    repoGestion.getResponseAsArray(
                            structureQueries.getComponentsForStructure(current.get(Constants.ID))));
        }
        return structures;
    }
    ...
    public JSONObject formatStructure(JSONObject structure, String id) throws RmesException {
        ...
        JSONArray componentDefinitionsFlat =
                repoGestion.getResponseAsArray(structureQueries.getComponentsForStructure(id));
        ...
        for (int i = 0; i < componentDefinitionsFlat.length(); i++) {
            ...
            JSONArray attachmentsArray = repoGestion.getResponseAsArray(structureQueries.getStructuresAttachments(
                    id, componentDefinitionFlat.getString(COMPONENT_DEFINITION_ID)));
            ...
            this.repoGestion.getMultipleTripletsForObject(
                    componentDefinitionFlat,
                    "contributor",
                    structureQueries.getComponentContributors(componentDefinitionFlat.getString(COMPONENT)),
                    "contributor");
    ...
    private void checkUnicityForStructure(Structure structure) throws RmesException {
        List<ComponentDefinition> componentsWithoutId = structure.getComponentDefinitions().stream()
                .filter((ComponentDefinition cd) -> cd.getComponent().getId() == null)
                .toList();

        if (componentsWithoutId.isEmpty()) {
            String[] ids = structure.getComponentDefinitions().stream()
                    .map(cd -> cd.getComponent().getId())
                    .map(Object::toString)
                    .toList()
                    .toArray(new String[0]);
            boolean structureWithSameComponents = ids.length > 0
                    && repoGestion.getResponseAsBoolean(structureQueries.checkUnicityStructure(structure.getId(), ids));
            if (structureWithSameComponents) {
                throw new RmesBadRequestException(
                        ErrorCodes.STRUCTURE_UNICITY, "A structure with the same components already exists", "");
            }
        }
    }

    public void createRdfStructure(
            Structure structure, String structureId, IRI structureIri, Resource graph, ValidationStatus status)
            throws RmesException {
        ...
        RdfUtils.addTripleString(
                structureIri, RDFS.COMMENT, structure.getDescriptionLg1(), languages.lg1(), model, graph);
        RdfUtils.addTripleString(
                structureIri, RDFS.COMMENT, structure.getDescriptionLg2(), languages.lg2(), model, graph);

        RdfUtils.addTripleUri(structureIri, DC.CREATOR, structure.getCreator(), model, graph);
        structure
                .getContributor()
                .forEach(contributor -> RdfUtils.addTripleUri(structureIri, DC.CONTRIBUTOR, contributor, model, graph));

        RdfUtils.addTripleUri(
                structureIri, INSEE.DISSEMINATIONSTATUS, structure.getDisseminationStatus(), model, graph);
    ...
    public void createRdfComponentSpecifications(
            IRI structureIRI, List<ComponentDefinition> componentList, Resource graph) throws RmesException {
    ...
    public void createRdfComponentSpecification(
            IRI structureIRI, ComponentDefinition componentDefinition, Resource graph) throws RmesException {
        Model model = new LinkedHashModel();

        IRI componentSpecificationIRI;

        componentSpecificationIRI =
                getComponentDefinitionIRI(RdfUtils.toString(structureIRI), componentDefinition.getId());
        ...
        model.add(
                componentSpecificationIRI,
                DCTERMS.IDENTIFIER,
                RdfUtils.setLiteralString(componentDefinition.getId()),
                graph);
        if (componentDefinition.getCreated() != null) {
            model.add(
                    componentSpecificationIRI,
                    DCTERMS.CREATED,
                    RdfUtils.setLiteralDateTime(componentDefinition.getCreated()),
                    graph);
        }
        if (componentDefinition.getModified() != null) {
            model.add(
                    componentSpecificationIRI,
                    DCTERMS.MODIFIED,
                    RdfUtils.setLiteralDateTime(componentDefinition.getModified()),
                    graph);
        }
        if (componentDefinition.getOrder() != null) {
            model.add(
                    componentSpecificationIRI, QB.ORDER, RdfUtils.setLiteralInt(componentDefinition.getOrder()), graph);
        }
        if (componentDefinition.getNotation() != null) {
            model.add(
                    componentSpecificationIRI,
                    SKOS.NOTATION,
                    RdfUtils.setLiteralString(componentDefinition.getNotation()),
                    graph);
        }
        if (componentDefinition.getLabelLg1() != null) {
            model.add(
                    componentSpecificationIRI,
                    RDFS.LABEL,
                    RdfUtils.setLiteralString(componentDefinition.getLabelLg1(), languages.lg1()),
                    graph);
        }
        if (componentDefinition.getLabelLg2() != null) {
            model.add(
                    componentSpecificationIRI,
                    RDFS.LABEL,
                    RdfUtils.setLiteralString(componentDefinition.getLabelLg2(), languages.lg2()),
                    graph);
        }
        MutualizedComponent component = componentDefinition.getComponent();
        ...
        if (component.getType().equals(RdfUtils.toString(QB.ATTRIBUTE_PROPERTY))) {
            ...
            model.add(
                    ...
                    RdfUtils.setLiteralBoolean(componentDefinition.getRequired()),
                    graph);
        }
    ...
    public String publishStructure(JSONObject structure) throws RmesException {
        PublicationUtils.rejectIfAlreadyPublished(
                "Structure", structure.optString(Constants.ID), structure.optString("validationState"));
        ...
        for (int i = 0; i < ids.length(); i++) {
            ...
            try {
                ...
                throw new RmesUnauthorizedException(
                        ...
                        "The component " + idComponent + " component can not be published",
                        new JSONArray());
            }
        }
