...
public class OperationsParentRepository extends RdfService {
    ...
    public OperationsParentRepository(
        ...
        if (target != null) {
            ...
            if (idOperation != null && !idOperation.isEmpty()) {
                ...
                throw new RmesException(
                        HttpStatus.SC_BAD_REQUEST, "Documentation has no target", "Check your documentation creation");
            }
        }
        return stamps;
    }
    ...
    public String getIndicatorsValidationStatus(String id) throws RmesException {
        try {
            return repoGestion
                    .getResponseAsObject(operationIndicatorsQueries.getPublicationState(id))
                    .getString("state");
    ...
    public String getFamOpSerValidationStatus(String id) throws RmesException {
        try {
            return repoGestion
                    .getResponseAsObject(operationQueries.getPublicationState(id))
                    .getString("state");
        } catch (JSONException _) {
            return Constants.UNDEFINED;
        }
    ...
    public JSONArray getSeriesCreators(IRI iri) throws RmesException {
        JSONArray raw = repoGestion.getResponseAsJSONList(
                operationSeriesQueries.getCreatorsBySeriesUri(RdfUtils.toString(iri)));
        return organisationLookup.canonicalize(raw);
    }
