...
public class IndicatorsRepository {
    ...
    public Indicator buildIndicatorFromJson(JSONObject indicatorJson, boolean forXML) {
        ...
        try {
            if (forXML)
                indicator = Deserializer.deserializeJsonString(
                        XMLUtils.solveSpecialXmlcharacters(indicatorJson.toString()), Indicator.class);
    ...
    private List<OperationsLink> buildListFromJsonToArray(JSONObject jsonIndicator, String constant) {
        ...
        List<Object> objects = operationsObjectMapper.buildObjectListFromJson(
                jsonIndicator.getJSONArray(constant), OperationsLink.getClassOperationsLink());
    ...
    private void addIndicatorCreators(String id, JSONObject indicator) throws RmesException {
        ...
        indicator.put(
                Constants.CREATORS,
                repositoryGestion.getResponseAsJSONList(operationIndicatorsQueries.getCreatorsById(id)));
    }
    ...
    private void addIndicatorPublishers(String id, JSONObject indicator) throws RmesException {
        ...
        indicator.put(
                ...
                repositoryGestion.getResponseAsJSONList(operationIndicatorsQueries.getPublishersById(id)));
    }
    ...
    private void addIndicatorContributors(String id, JSONObject indicator) throws RmesException {
        ...
        indicator.put(
                ...
                repositoryGestion.getResponseAsJSONList(operationIndicatorsQueries.getContributorsById(id)));
    }
    ...
    private void addOneTypeOfLink(String id, JSONObject object, IRI predicate) throws RmesException {
        JSONArray links =
                repositoryGestion.getResponseAsArray(operationIndicatorsQueries.indicatorLinks(id, predicate));
        if (!links.isEmpty()) {
    ...
    public void setIndicator(String id, String body) throws RmesException {
        ...
        documentationsUtils.updateDocumentationTitle(
                indicator.getIdSims(), indicator.getPrefLabelLg1(), indicator.getPrefLabelLg2());
    ...
    public void addMulltiLangValues(
            Model model, IRI indicatorIRI, Resource graph, String valueLg1, String valueLg2, IRI predicate) {
    ...
    void createRdfIndicator(Indicator indicator, ValidationStatus newStatus) throws RmesException {
        ...
        model.add(
                indicURI, ADMS.HAS_IDENTIFIER, RdfUtils.setLiteralString(indicator.getId()), RdfUtils.productsGraph());
        /*Required*/
        model.add(
                indicURI,
                SKOS.PREF_LABEL,
                RdfUtils.setLiteralString(indicator.getPrefLabelLg1(), languages.lg1()),
        ...
        RdfUtils.addTripleString(
                indicURI,
                SKOS.PREF_LABEL,
                indicator.getPrefLabelLg2(),
                languages.lg2(),
                model,
                RdfUtils.productsGraph());
        RdfUtils.addTripleString(
                indicURI, SKOS.ALT_LABEL, indicator.getAltLabelLg1(), languages.lg1(), model, RdfUtils.productsGraph());
        RdfUtils.addTripleString(
                indicURI, SKOS.ALT_LABEL, indicator.getAltLabelLg2(), languages.lg2(), model, RdfUtils.productsGraph());
        RdfUtils.addTripleDateTime(
                indicURI, DCTERMS.CREATED, indicator.getCreated(), model, RdfUtils.operationsGraph());
        RdfUtils.addTripleDateTime(
                indicURI, DCTERMS.MODIFIED, indicator.getUpdated(), model, RdfUtils.operationsGraph());

        addMulltiLangValues(
                model,
                indicURI,
                RdfUtils.productsGraph(),
                indicator.getAbstractLg1(),
                indicator.getAbstractLg2(),
                DCTERMS.ABSTRACT);
        addMulltiLangValues(
                model,
                indicURI,
                RdfUtils.productsGraph(),
                indicator.getHistoryNoteLg1(),
                indicator.getHistoryNoteLg2(),
                SKOS.HISTORY_NOTE);
        ...
        String accPeriodicityUri = codeListService.getCodeUri(
                indicator.getAccrualPeriodicityList(), indicator.getAccrualPeriodicityCode());
    ...
    void addOrganisationLinks(List<String> orgRefs, IRI predicate, Model model, IRI indicURI, Resource graph)
            throws RmesException {
        if (orgRefs == null) {
            return;
        }
