...
public class CreateCollectionCommand {
    ...
    private final List<String> conceptsIdendifiers;

    public CreateCollectionCommand(LocalisedLabel defaultLabel, @Nullable LocalisedLabel alternativeLabel, Map<Lang, String> descriptions, String creator, @Nullable String contributor, List<String> conceptsIdendifiers) throws InvalidCreateCollectionCommandException {
        if(StringUtils.isAllBlank(creator)){
            throw new InvalidCreateCollectionCommandException("The creator is blank");
        }
        if(conceptsIdendifiers.stream().anyMatch(StringUtils::isAllBlank)){
        ...
        this.conceptsIdendifiers = conceptsIdendifiers;
    }
