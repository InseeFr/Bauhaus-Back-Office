...
public class DocumentsResources {
    ...
    public ResponseEntity<String> setDocument(
            @RequestParam(value = "body") String body, @RequestParam(value = "file") MultipartFile documentFile)
    ...
    public ResponseEntity<String> setDocument(@PathVariable(Constants.ID) DocumentId id, @RequestBody String body)
    ...
    public ResponseEntity<String> changeDocument(
            @RequestParam(value = "file") MultipartFile documentFile, @PathVariable(Constants.ID) String id)
            throws RmesException, IOException {
        String documentName = documentFile.getOriginalFilename();
    ...
    public ResponseEntity<Object> deleteDocument(@PathVariable(Constants.ID) DocumentId id) throws RmesException {
        String documentIdString = (id.getDocumentId() != null) ? sanitizeDocumentId(id.getDocumentId()) : null;
        return ResponseEntity.status(documentsService.deleteDocument(documentIdString))
                .body(documentIdString);
    }
    ...
    public ResponseEntity<String> getLink(@PathVariable(Constants.ID) String id) throws RmesException {
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .contentType(MediaType.APPLICATION_JSON)
                .body(documentsService.getLink(id).toString());
    }
    ...
    @HasAccess(module = RBAC.Module.OPERATION_DOCUMENT, privilege = RBAC.Privilege.CREATE)
    public ResponseEntity<Object> setLink(@RequestParam(value = "body") String body) throws RmesException, IOException {
        return ResponseEntity.status(HttpStatus.OK).body(documentsService.setLink(body));
    }

    @PutMapping("/link/{id}")
    @HasAccess(module = RBAC.Module.OPERATION_DOCUMENT, privilege = RBAC.Privilege.UPDATE)
    public ResponseEntity<Object> setLink(@PathVariable(Constants.ID) DocumentId id, @RequestBody String body)
            throws RmesException {
        String documentIdString = (id.getDocumentId() != null) ? sanitizeDocumentId(id.getDocumentId()) : null;
    ...
    @HasAccess(module = RBAC.Module.OPERATION_DOCUMENT, privilege = RBAC.Privilege.DELETE)
    public ResponseEntity<Object> deleteLink(@PathVariable(Constants.ID) DocumentId id) throws RmesException {
        String documentIdString = (id.getDocumentId() != null) ? sanitizeDocumentId(id.getDocumentId()) : null;
        return ResponseEntity.status(documentsService.deleteLink(documentIdString))
                .body(documentIdString);
    }
    ...
    private void verifyExtension(String fileName) throws RmesException {
        ...
        if (fileNameElements.length < 2) {
            ...
            boolean isKnownActualExtension = Arrays.stream(extensionsExpected)
                    .anyMatch(extensionExpected -> extensionExpected.equals(extensionsActual));
