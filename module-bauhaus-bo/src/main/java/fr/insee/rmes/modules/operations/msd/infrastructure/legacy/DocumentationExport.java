...
public class DocumentationExport {
    ...
    public DocumentationExport(
            ...
            CodesListExport codeListServiceImpl,
    ...
    public ResponseEntity<Resource> exportAsResponse(
        ...
        if (documents) {
            exporter = (xml, xsl, xmlPattern, zip, documentation) ->
                    exportAsZip(sims, xml, xsl, xmlPattern, zip, documentation, maxLength);
        } else {
            String fileName =
                    FilesUtils.generateFinalFileNameWithoutExtension(sims.getString(Constants.LABEL_LG1), maxLength);
            exporter = (xml, xsl, xmlPattern, zip, documentation) ->
                    exportUtils.exportAsODT(fileName, xml, xsl, xmlPattern, zip, documentation);
        }
        return export(exporter, xmlContent, patternAndZip);
    }

    public ResponseEntity<Resource> exportAsZip(
            JSONObject sims,
            Map<String, String> xmlContent,
            String xslFile,
            String xmlPattern,
        ...
        try {
            ...
            Resource resource = new UrlResource(
                    Paths.get(simsDirectory.toString(), simsDirectory.getFileName() + FilesUtils.ZIP_EXTENSION)
                            .toUri());
            ...
            if (!history.contains(url)) {
                ...
                if (!documentsUtils.existsInStorage(documentFilename)) {
                    ...
                    String documentFileName =
                            FilesUtils.generateFinalFileNameWithExtension(UriUtils.getLastPartFromUri(url), maxLength);
                    try (InputStream inputStream = documentsUtils.retrieveDocumentFromStorage(documentFilename)) {
                        ...
                        logger.debug(
                                "Writing the document {} with the name {} into the folder {}",
                                url,
                                documentFileName,
                                directory);
                        Path documentTempFile =
                                Files.createFile(Path.of(documentDirectory.toString(), documentFileName));
    ...
    public ResponseEntity<Object> exportXmlFiles(
            Map<String, String> xmlContent, String targetType, boolean includeEmptyFields, boolean lg1, boolean lg2)
            throws RmesException {
    ...
    public ResponseEntity<Resource> exportMetadataReport(
            String id, Boolean includeEmptyMas, Boolean lg1, Boolean lg2, Boolean document, String goal, int maxLength)
            throws RmesException {
    ...
    public ResponseEntity<Object> exportMetadataReportFiles(
            String id, Boolean includeEmptyMas, Boolean lg1, Boolean lg2) throws RmesException {
        Map<String, String> xmlContent = new HashMap<>();
        String targetType = getXmlContent(id, xmlContent);
    ...
    public String getXmlContent(String id, Map<String, String> xmlContent) throws RmesException {
        ...
        if (targetType.equals(Constants.INDICATOR_UP)) {
            ...
            String idSeries = XMLUtils.getTagValues(
                            XMLUtils.getTagValues(indicatorXML, Constants.WASGENERATEDBY)
                                    .getFirst(),
                            Constants.ID)
                    .getFirst();
            series = seriesRepository.getSeriesById(idSeries, EncodingType.XML);
    ...
    private List<String> transformStampsToLabels(List<String> stamps) throws RmesException {
        ...
        for (String stamp : stamps) {
            ...
            if (organisation != null
                ...
                // If organization not found or has no value, keep a readable form (last segment of an IRI, or the raw
                // stamp).
        ...
        logger.debug(
                "Transformation complete: {} succeeded, {} not found (kept original stamps)",
                successCount,
                notFoundCount);
    ...
    private static String shortenIfIri(String stamp) {
        ...
        return lastSlash >= 0 && lastSlash < stamp.length() - 1 ? stamp.substring(lastSlash + 1) : stamp;
    }
    ...
    private interface Exporter {
        ResponseEntity<Resource> export(
                Map<String, String> xmlContent, String xslFile, String xmlPattern, String zip, String objectType)
                throws RmesException;
    }
