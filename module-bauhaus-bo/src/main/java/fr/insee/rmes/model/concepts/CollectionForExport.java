...
public class CollectionForExport {
    ...
    public void addMembers(JSONArray members) {
        JSONUtils.stream(members).forEach(member -> {
            ...
            if (member.has(Constants.EDITORIAL_NOTE_LG1))
                rep.setEditorialNoteLg1(member.getString(Constants.EDITORIAL_NOTE_LG1));
            if (member.has(Constants.EDITORIAL_NOTE_LG2))
                rep.setEditorialNoteLg2(member.getString(Constants.EDITORIAL_NOTE_LG2));
            if (member.has(Constants.ISVALIDATED))
                rep.setIsValidated(ExportUtils.toValidationStatus(member.getString(Constants.ISVALIDATED), true));
