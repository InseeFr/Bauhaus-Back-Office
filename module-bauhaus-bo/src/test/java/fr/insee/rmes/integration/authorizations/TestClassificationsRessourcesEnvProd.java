...
@Import({ClassificationsResources.class, UserAuthTestConfiguration.class})
class TestClassificationsRessourcesEnvProd extends AbstractResourcesEnvProd {
    ...
    void getClassificationsFamilies(String url, Integer code, boolean hasAccessReturn)
    ...
    void getClassificationsSeries(String url, Integer code, boolean hasAccessReturn)
            throws Exception, MissingUserInformationException {
        when(checker.hasAccess(any(), any(), any(), any())).thenReturn(hasAccessReturn);
    ...
    void getClassifications(String url, Integer code, boolean hasAccessReturn)
            throws Exception, MissingUserInformationException {
        when(checker.hasAccess(any(), any(), any(), any())).thenReturn(hasAccessReturn);
    ...
    static Collection<Arguments> testClassificationGetEndpoint() {
        return Arrays.asList(
                ...
                Arguments.of(
                        "/classifications/classification/" + familyId + "/level/" + levelId + "/members", 200, true),
                Arguments.of(
                        "/classifications/classification/" + familyId + "/level/" + levelId + "/members", 403, false),
                ...
                Arguments.of(
                        "/classifications/classification/" + familyId + "/item/" + itemId + "/notes/" + conceptVersion,
                        200,
                        true),
                Arguments.of(
                        "/classifications/classification/" + familyId + "/item/" + itemId + "/notes/" + conceptVersion,
                ...
                Arguments.of(
                        "/classifications/classification/" + familyId + "/item/" + itemId + "/narrowers", 200, true),
                Arguments.of(
                        "/classifications/classification/" + familyId + "/item/" + itemId + "/narrowers", 403, false),
                ...
                Arguments.of(
                        "/classifications/correspondence/" + correspondenceId + "/association/" + associationId,
                        200,
                        true),
                Arguments.of(
                        "/classifications/correspondence/" + correspondenceId + "/association/" + associationId,
    ...
    void publishClassification(Integer code, boolean hasAccessReturn)
            throws Exception, MissingUserInformationException {
        when(checker.hasAccess(any(), any(), any(), any())).thenReturn(hasAccessReturn);

        configureJwtDecoderMock(jwtDecoder, idep, timbre, Collections.emptyList());

        var request = put("/classifications/classification/" + familyId + "/validate")
    ...
    static Collection<Arguments> TestRoleCaseForPublishClassification() {
        return Arrays.asList(Arguments.of(200, true), Arguments.of(403, false));
    }

    @ParameterizedTest
    @MethodSource("TestRoleCaseForUploadClassification")
    void updateClassificationItemWhenAnyRole(Integer code, boolean hasAccessReturn)
            throws Exception, MissingUserInformationException {
        when(checker.hasAccess(any(), any(), any(), any())).thenReturn(hasAccessReturn);
