...
public class TokenForTestsConfiguration {
    ...
    public static void configureJwtDecoderMock(
            JwtDecoder jwtDecoderMock, String idep, String timbre, List<String> roles) {
        when(jwtDecoderMock.decode(anyString()))
                .then(invocation -> Jwt.withTokenValue(invocation.getArgument(0))
                        .header("typ", "JWT")
                        .header("alg", "none")
                        .claim(ID_CLAIM, idep)
