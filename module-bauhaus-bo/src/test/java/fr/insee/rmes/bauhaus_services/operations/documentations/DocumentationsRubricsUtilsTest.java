...
class DocumentationsRubricsUtilsTest {
    ...
    void setUp() {
        ...
        RdfUtils.setBauhausUriBuilder(new BauhausUriBuilder(
                "http://id.insee.fr/", "http://rdf.insee.fr/graphes/", p -> Optional.of("/rapport")));
    }
    ...
    void shouldAddRichTextRubricToModelWithSimpleAddTripleString() throws RmesException {
        ...
        assertTrue(
                model.contains(textUriLg1, RDF.TYPE, DCMITYPE.TEXT),
                "Le modèle devrait contenir le type TEXT pour texte (lg1)");
        assertTrue(
                model.contains(textUriLg1, DCTERMS.LANGUAGE, factory.createIRI(languageUris.lang1Uri())),
        ...
        assertTrue(
                model.contains(textUriLg2, RDF.TYPE, DCMITYPE.TEXT),
                "Le modèle devrait contenir le type TEXT pour text (lg2)");
        assertTrue(
                model.contains(textUriLg2, DCTERMS.LANGUAGE, factory.createIRI(languageUris.lang2Uri())),
    ...
    void shouldAddSimpleTextRubricToModel() throws RmesException {
        ...
        assertTrue(
                model.contains(attributeUri, RDF.TYPE, SDMX_MM.REPORTED_ATTRIBUTE),
                "Le modèle devrait contenir le type REPORTED_ATTRIBUTE");

        // Vérifie que les valeurs textuelles ont été ajoutées
        boolean hasTextValue = model.stream().anyMatch(stmt -> stmt.getSubject().equals(attributeUri));
    ...
    void shouldNotAddRichTextWhenRubricIsEmpty() throws RmesException {
        ...
        assertTrue(
                model.contains(attributeUri, SDMX_MM.METADATA_REPORT_PREDICATE, simsUri),
                "Le modèle devrait contenir le lien vers le metadata report");

        // Vérifie qu'aucun TEXT n'a été créé
        long textCount = model.stream()
                .filter(stmt ->
                        stmt.getPredicate().equals(RDF.TYPE) && stmt.getObject().equals(DCMITYPE.TEXT))
                .count();
    ...
    void shouldAddDateRubricToModel() throws RmesException {
        ...
        boolean hasDateValue =
                model.stream().anyMatch(stmt -> stmt.getPredicate().toString().contains("dateAttribute"));
    ...
    void shouldAddCodeListRubricToModel() throws RmesException {
        ...
        boolean hasCodeUri =
                model.stream().anyMatch(stmt -> stmt.getObject().stringValue().equals(codeUri));
    ...
    void shouldAddGeographyRubricToModel() throws RmesException {
        ...
        boolean hasGeoUri =
                model.stream().anyMatch(stmt -> stmt.getObject().stringValue().equals(geoUri));
    ...
    void shouldThrowBadRequestWhenIdAttributeNotFoundInMsd() throws RmesException {
        ...
        assertTrue(
                exception.getDetails().contains(unknownAttributeId),
    ...
    void shouldBuildRubricFromJsonWithRichTextAndMarkdownConversion() {
        ...
        assertTrue(
                result.getLabelLg1().contains("replacementForInf")
                        && result.getLabelLg1().contains("h1"),
                "Le label devrait contenir du HTML échappé avec des marqueurs de remplacement: "
                        + result.getLabelLg1());
        assertTrue(
                result.getLabelLg2().contains("replacementForInf")
                        && result.getLabelLg2().contains("h1"),
                "Le label devrait contenir du HTML échappé avec des marqueurs de remplacement: "
                        + result.getLabelLg2());
    }
    ...
    void shouldBuildRubricFromJsonWithSimpleText() {
        ...
        assertTrue(
                result.getLabelLg1().contains("Titre") || result.getLabelLg1().equals("Titre simple"));
    }
