...
public class ForbiddenApiArchTest {
    ...
    public static final ArchRule noLanguagePropertyInjectedWithValue = noCodeUnits()
            ...
            .because("two String parameters can be swapped without the compiler noticing: inject the BauhausLanguagesProperties object instead");
            ...
            public void check(JavaCodeUnit codeUnit, ConditionEvents events) {
                for (JavaParameter parameter : codeUnit.getParameters()) {
                    parameter.getAnnotations().stream()
                            ...
                            .filter(expression -> expression.contains(LG1_PROPERTY) || expression.contains(LG2_PROPERTY))
                            .forEach(expression -> events.add(SimpleConditionEvent.satisfied(codeUnit,
                                    codeUnit.getFullName() + " injects " + expression)));
                }
            }
        };
    }
