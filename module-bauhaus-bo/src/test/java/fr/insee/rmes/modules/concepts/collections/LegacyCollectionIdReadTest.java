...
class LegacyCollectionIdReadTest extends WithGraphDBContainer {
    ...
    void listing_collections_does_not_500_on_legacy_id_with_underscore() {
        ...
        boolean foundLegacy =
                JSONUtils.stream(rows).anyMatch(row -> "Legacy_collection_001".equals(row.getString("id")));
        assertThat(foundLegacy)
                .as(
                        "La collection legacy 'Legacy_collection_001' (ID avec underscore) doit être listée comme les autres")
                .isTrue();
    }
}
