...
public class ColecticaRepositoryConfiguration {
    ...
    public ColecticaClient colecticaClient(
            ColecticaConfiguration colecticaConfiguration, @Qualifier("colectica") TokenService colecticaTokenService) {
        var server = colecticaConfiguration.server();
        ColecticaCredentials credentials = "token".equals(server.authenticationMode())
        ...
        return switch (mutualizedCodesProperties.mutualizedCodesStrategy()) {
            case CONFIGURED_GROUPS -> {
                ...
                yield new ConfiguredGroupsCodeListRefsProvider(
                        colecticaConfiguration.server(), groupRefs, colecticaClient);
            }
            case PACKAGE_WALK ->
                new MutualizedCodeListRefsProvider(
                        colecticaConfiguration.server(), colecticaConfiguration, colecticaClient);
        };
    }
    ...
    public GroupRepository groupRepository(
        ...
        return new ColecticaGroupRepository(
                colecticaClient, colecticaConfiguration.server(), ddi4ToLifecycle33, ddiRepository);
    }
