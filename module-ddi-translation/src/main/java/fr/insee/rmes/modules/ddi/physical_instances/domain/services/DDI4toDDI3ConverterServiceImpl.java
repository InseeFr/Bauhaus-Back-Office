...
public class DDI4toDDI3ConverterServiceImpl implements DDI4toDDI3ConverterService {
     ...
     */
    /**
     * Relit l'attribut DDI3 {@code @versionDate} porte par la Map additionalProperties d'un POJO
     * genere (cf. {@code Ddi3XmlReader#putDdi3Attributes}).
     */
        ...
        if (ddi4.physicalInstance() != null) {
            ddi4.physicalInstance().forEach(pi -> {
                try {
                    ...
                    items.add(createDdi3Item(itemTypes.get("PhysicalInstance"), pi.getAgency(), pi.getVersion(),
                            pi.getID(), xmlFragment, versionDate(pi.getAdditionalProperties())));
        ...
        if (ddi4.dataRelationship() != null) {
            ddi4.dataRelationship().forEach(dr -> {
                try {
                    ...
                    items.add(createDdi3Item(itemTypes.get("DataRelationship"), dr.getAgency(), dr.getVersion(),
                            dr.getID(), xmlFragment, versionDate(dr.getAdditionalProperties())));
                } catch (XMLStreamException e) {
        ...
        if (ddi4.variable() != null) {
            ddi4.variable().forEach(var -> {
                try {
                    ...
                    items.add(createDdi3Item(itemTypes.get("Variable"), var.getAgency(), var.getVersion(),
                            var.getID(), xmlFragment, versionDate(var.getAdditionalProperties())));
                } catch (XMLStreamException e) {
        ...
        if (ddi4.codeList() != null) {
            ddi4.codeList().forEach(cl -> {
                try {
                    ...
                    items.add(createDdi3Item(itemTypes.get("CodeList"), cl.getAgency(), cl.getVersion(),
                            cl.getID(), xmlFragment, versionDate(cl.getAdditionalProperties())));
                } catch (XMLStreamException e) {
        ...
        if (ddi4.category() != null) {
            ddi4.category().forEach(cat -> {
                try {
                    ...
                    items.add(createDdi3Item(itemTypes.get("Category"), cat.getAgency(), cat.getVersion(),
                            cat.getID(), xmlFragment, versionDate(cat.getAdditionalProperties())));
                } catch (XMLStreamException e) {
